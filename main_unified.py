...
def load_vision_models():
    ...
    print("⏳ [LAZY LOAD] Memuat model Vision & Database Fitur ke RAM...")
    ...
    if os.path.exists(MODEL_PATH) and os.path.exists(CSV_PATH) and os.path.exists(PKL_PATH):
        ...
        vision_loaded = True
        print("✅ Vision Model & Database Loaded!")
    else:
        print("❌ File Vision Model atau Database Fitur tidak lengkap.")
...
def recommend_visual(file: UploadFile = File(...)):
    ...
    try:
        print("\n--- 🔍 MEMULAI PROSES KLASIFIKASI & PENCARIAN REKOMENDASI ---")
        img_bytes = file.file.read()
        processed_img = preprocess_image(img_bytes)
        recommendations = []

        if model is None or feature_extractor is None or df_features is None or df_csv is None:
            return {"status": "error", "message": "Model atau database belum termuat dengan sempurna"}
